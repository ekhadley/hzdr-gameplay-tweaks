...
- **Concentration sensitivity — weapon wheel over-boost (fix implemented, pending in-game validation):** the correction now gates on `m_WorldTimeScale < 1` **and** the player actually aiming, not on timescale alone. The weapon wheel also slows the world but isn't aiming, so it no longer over-boosts the mouse. Aiming is read as bit 0 of the dword at `controller + 0x1CB8` (controller = `Player + 0x138`): set during aim and concentration, clear during the weapon wheel. The offset was found by attaching to the live process (scanmem + `process_vm_readv`) and diffing the controller's int32 fields across aim / concentration / not-aiming / weapon-wheel states. The earlier `+0x47D8 & 1` flag read 0 during concentration, which is why it failed.

- **Concentration sensitivity feels "off" vs. original (non-remaster) HZD:** the correction works, but the feel during concentration isn't quite the same as the original game. Symptom only; cause unconfirmed. The correction is live-tracked (`k = 1 + correction*(1/m_WorldTimeScale − 1)` recomputed per frame, so it ramps smoothly with the slow-mo ease-in/out), so a binary toggle is *not* the cause. FOV is ruled out as the (sole) cause: the same weird feel was observed during **weapon-wheel** concentration, which does not change FOV (this observation predates the aim gate above; the correction no longer runs during the weapon wheel). The leading hunch is a **timing mismatch**: a delay between when concentration actually ends and when we stop applying (or fully unwind) the correction. Possible explanations, roughly in order of suspicion:
